...
import {
  ...
  X, MapPin, Locate, Check, Pen, Home,
...
export default function EditAddressModel({ selectedAddress, editAddress, setEditModal, setSelectedAddress, loading }) {
  ...
  const handlePincodeChange = async (val) => {
    setSelectedAddress((prev) => ({ ...prev, pincode: val }));
    setPinStatusMsg("");
    ...
    <div className="bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-2xl p-5 sm:p-6 space-y-4 max-h-[85vh] overflow-y-auto">
      ...
      <div className="flex justify-between items-center pb-2 border-b border-gray-100 dark:border-gray-800">
        <h2 className="text-xl font-bold flex items-center gap-2 text-gray-900 dark:text-white">
          <MapPin className="w-5 h-5 text-[#1E88E5]" /> Edit Delivery Address
        </h2>
        <button
          type="button"
      ...
      <form onSubmit={editAddress} className="space-y-3.5 text-xs sm:text-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input
              type="text"
              value={selectedAddress?.city || selectedAddress?.district || ""}
              placeholder="City / District (Auto-filled)"
              className="w-full p-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500"
              onChange={(e) => setSelectedAddress({ ...selectedAddress, city: e.target.value, district: e.target.value })}
              required
            />
          </div>
        </div>
        ...
        <div>
          ...
          <input
            ...
            className="w-full p-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500"
            onChange={(e) => setSelectedAddress({ ...selectedAddress, village: e.target.value })}
          />
        ...
        <div>
          ...
          <input
            type="text"
            value={selectedAddress?.hno || ""}
            placeholder="e.g. 8-137 or Flat 101"
            className="w-full p-2.5 rounded-xl border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-white font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500"
            onChange={(e) => setSelectedAddress({ ...selectedAddress, hno: e.target.value })}
          />
        </div>
