  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white dark:bg-gray-800 p-3 shadow-xl rounded-xl border border-gray-200 dark:border-gray-700 text-xs space-y-1">
        ...
        <p className="text-[#6C5CE7] font-semibold flex items-center justify-between gap-3">
          <span>Revenue:</span>
          <span>₹{formatNumberWithCommas(payload[0]?.value || 0)}</span>
        </p>
        <p className="text-purple-600 font-semibold flex items-center justify-between gap-3">
          <span>Profit:</span>
          <span>₹{formatNumberWithCommas(payload[1]?.value || 0)}</span>
        </p>
        <p className="text-purple-400 font-semibold flex items-center justify-between gap-3">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-gray-100 dark:border-gray-700/60 pb-3">
        <div>
          <h2 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
            <ShowChartIcon className="text-[#6C5CE7] dark:text-purple-300" />
        ...
        <div className="flex items-center gap-2 bg-purple-50 dark:bg-purple-950/40 border border-purple-200 dark:border-purple-800 text-[#6C5CE7] dark:text-purple-300 text-xs px-3 py-1 rounded-full font-bold self-start sm:self-auto">
      ...
      <div className="h-72 w-full pt-2">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={timelineData} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="revenue" name="Revenue (₹)" stroke="#6C5CE7" strokeWidth={2.5} fillOpacity={1} fill="url(#colorRevenue)" />
            <Area type="monotone" dataKey="profit" name="Net Profit (₹)" stroke="#8B5CF6" strokeWidth={2} fillOpacity={1} fill="url(#colorProfit)" />
            <Area type="monotone" dataKey="sales" name="Sales (Units)" stroke="#A78BFA" strokeWidth={2} fillOpacity={1} fill="url(#colorSales)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </div>
